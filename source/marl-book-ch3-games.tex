...
\begin{frame}{Normal-Form Games}
    ...
    \vspace{10pt}
    \blist
        \item $I$ is a finite set of agents \(I = \{1, ..., n\}\)
...
\begin{frame}{Normal-Form Game Process}
    ...
    \begin{enumerate}
        ...
        \pause
        \item<4-> Each agent receives a reward based on their \fat{individual} reward function and the \fat{joint action}, \(r_i = \mathcal{R}_i(a)\)
    \end{enumerate}
...
\begin{frame}{Classes of Games}
    ...
    \blist
        \item In \fat{zero-sum games}, the sum of the agents' reward is always 0\\
        i.e. \(\sum_{i \in I} \mathcal{R}_{i} (a) = 0, \forall a \in A\)
        \item In \fat{common-reward} games, all agents receive the same reward 
        (\(R_i = R_j ; \forall i, j \in I\))
        \item In \fat{general-sum} games, there are no restrictions on the relationship between reward functions. 
    \elist
...
\begin{frame}{Matrix Games}
    ...
    \begin{figure}[ht]
        ...
        \vspace{5pt}
        \begin{flushleft}
            \visible<5->{Note general sum is a \fat{superset} class for all games}
        \end{flushleft}
    \end{figure}
\end{frame}
...
\begin{frame}{Stochastic Games}
    ...
    Stochastic games introduce the notion of \fat{states} and are defined as a 6 tuple \((I, S, \{A_i\}_{i \in I}, \{\mathcal{R}_i\}_{i \in I}, \mathcal{T}, \mu)\)
    \vspace{10pt}
    \blist
...
\begin{frame}{Stochastic Game Level-Based Foraging Example}
...
\begin{frame}{Partially Observable Stochastic Games (POSG)}
    \vspace{5pt}
    \blist
        \item At the top of the game model hierarchy, the most \fat{general} model is the POSG
...
\begin{frame}{Belief States}
    ...
    \vspace{10pt}
        \blist 
            ...
            \item Agent 1 can hold a \fat{belief states} \(b_{i}^{t}\), providing a probability distribution over possible state \(s \in S\)
...
\visible<2->{
    In MARL this type of update is typically \fat{unfeasible}:
    \blist
...
\blist
    \item In MARL, we assume \fat{limited knowledge}, i.e. no knowledge of the $\mathcal{T}$ and no knowledge of other agents $\mathcal{R}$
