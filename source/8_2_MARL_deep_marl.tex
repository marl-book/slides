...
\begin{frame}[t]{Recap: Tabular Agent Modeling}
    ...
    \begin{solutionbox}
        As for value functions, we can use \fat{deep learning} to learn generalisable agent models!
    \end{solutionbox}
\end{frame}
...
\begin{frame}[t]{Joint-Action Learning with \fat{Deep} Agent Models}
    ...
    \blist
        ...
        \item<2-> Given the observation history of agent $i$, its model $\agmod^i_{j}(\ac_{j} \mid \his_i; \phi^i_j)$ for agent $j$ outputs a probability distribution over its actions
...
\begin{frame}[t]{Learning Compact Representations of Agent Policies}
    ...
    \bmath
        \loss(\psi_i^e, \psi_i^d) = \sum_{j\neq i} -\log \hat{\pol}_j^{i,t}(\jac_j^t) \text{\ with\ } \hat{\pol}_j^{i,t} = f^d\left(f^e(\his_i^t; \psi_i^e); \psi_i^d\right)_j
    \emath
...
\begin{frame}[t]{Parameter Sharing}
    Sharing network parameters across agents is a common practice to make MARL training more efficient. We can share parameters across value functions, policies, or both.

    \only<1>{
        \begin{figure}
            \centering
            \includegraphics[height=14em]{figures/parameter_sharing_mix.pdf}
        \end{figure}
    }
    ...
    \only<2->{
        ...
        \begin{minipage}{.3\textwidth}
            \begin{figure}
                \includegraphics[height=14em]{figures/parameter_sharing_cut.pdf}
            \end{figure}
        \end{minipage}
...
\begin{frame}[t]{Parameter Sharing in LBF}
    \begin{figure}
        ...
        \begin{subfigure}[b]{.39\textwidth}
            \centering
            \raisebox{15pt}{\includegraphics[width=.8\textwidth]{figures/lbf_6x6-2p-1f.jpg}}
            \caption{Environment}
        \end{subfigure}
        \visible<2->{
            \begin{subfigure}[b]{.59\textwidth}
...
\begin{frame}[t]{The Challenge of Zero-Sum Board Games}
    ...
    \only<1>{
        \begin{figure}
            ...
            \begin{subfigure}{.49\textwidth}
                \centering
                \includegraphics[width=.7\textwidth]{figures/go_board}
                \caption{Go}
            \end{subfigure}
        \end{figure}
    }
...
\begin{frame}[t]{Tree Search for Zero-Sum Games}
    ...
    \begin{minipage}{.6\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{figures/game-tree.pdf}
        \end{figure}
    \end{minipage}
    \hfill
    \begin{minipage}{.35\textwidth}
        \only<2>{
            ...
            \blist
                ...
                \item Edges represent actions
                \item Leaves represent terminal states
            \elist
            and in each node either agent $1$ or agent $2$ makes a move.
        }
        \only<3>{
            \begin{problembox}
...
\begin{frame}[t]{Monte Carlo Tree Search (MCTS)}
    ...
    \e{Monte Carlo Tree Search} (MCTS) is a commonly used tree-search algorithm based on simulating possible outcomes of the game, and then updating value estimates of actions.
...
\begin{frame}[t]{AlphaZero -- Self-Play MCTS with Deep Learning}
    ...
    \begin{minipage}{.29\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{figures/nature_alphago}
        \end{figure}
    \end{minipage}
\end{frame}
...
\begin{frame}[t]{Population-Based Training -- Overview}
    ...
    \blist
        \item \fat{Evaluation:} evaluate the performance of the policies in the population by playing games against policies of the current populations of all other agents
        \item \fat{Evolution:} based on evaluation results, evolve the populations of all agents. This can be done by selecting a subset of high-performing policies, mutating existing policies, or adding new policies to the population.
    \elist
    ...
    \begin{columns}
        \begin{column}{0.65\textwidth}
            \begin{enumerate}
                ...
                \item<3-> Construct a \fat{meta-game} $M^k$ at generation $k$ from the populations of all agents as a stateless single-step general-sum game with
                    \blist
...
\begin{frame}[t]{AlphaStar -- GrandMaster in StarCraft II}
    ...
    \only<1>{
        \begin{figure}
            \centering
            \includegraphics[width=.6\textwidth]{figures/alphastar_starcraft}
