...
\begin{frame}[t]{Convolutional Neural Networks}
    ...
    \e{Convolutional neural networks} (CNNs) are neural networks designed for image processing.
    \blist
    ...
    \blist
        ...
        \elist
        \item \fat{Exploiting spatial correlations:} nearby pixels are likely related (e.g., edges), and the same spatial patterns may occur at different locations of an image
    \elist
...
\only<2>{
\begin{figure}
    \includegraphics[width=0.4\textwidth]{images/chapter_7/cnn.pdf}
\end{figure}
}
...
\begin{minipage}{0.48\textwidth}
    \only<2->{
        \blist
          \item A convolution of a filter with parameters $W \in \mathbb{R}^{d_w \times d_w}$ over input $x \in \mathbb{R}^{d_x \times d_x}$ is:
                \[
                    y_{i, j} = \sum_{a=1}^{d_w}\sum_{b=1}^{d_w} W_{a,b} x_{i+a-1, j+b-1}
                \]
        \elist
