...
\begin{problembox}
    The \e{moving target problem} arises from the bootstrapped targets:
    \vspace{-1em}
    \[
...
\visible<3->{
    \begin{solutionbox}
        One potential solution is to use a \textbf{target network} with parameters $\bar{\theta}$, which are updated less frequently than our $Q$ network's parameters $\theta$  
    \end{solutionbox}
}
...
\visible<3->{
    \begin{solutionbox}
        We train on samples of previous experiences stored in a \textbf{replay buffers}.
    \end{solutionbox}
}
...
\blist
    ...
    \item<2-> These preferences are then transformed into a probability distribution across the action space using a \fat{softmax} function:
\elist
...
\blist
    ...
    \item<3-> Policy gradients are also effective for representing continuous action spaces
\elist
...
\only<1>{
\blist
    \item $J$ represents a function measuring the quality policy $\pi$
    ...
    \item The $J$ function is similar to a loss function with the key difference that we aim to \textbf{maximize} rather than minimize it
\elist
}

\only<2->{
\begin{notebox}
    The policy gradient theorem assumes that $Pr(s \mid \pi)$ are given under the currently optimized policy $\pi$ $\rightarrow$ needs \fat{on-policy} data
\end{notebox}
}
...
\fat{Next we'll cover:}
    ...
    \blist
        \item Deep \fat{multi agent} reinforcement learning 
    \elist
