...
\begin{frame}{Correlated Equilibrium}
    ...
    \blist
        \item \fat{Correlated equilibria}, allow for correlated policies
        ...
        \item then a \fat{correlated equilibrium} can be defined as:
    \elist
    \vspace{0pt}
    \begin{equation*}
...
\begin{flushleft}
\only<1>{Non-Correlated Nash Equilibrium:}
\only<2>{Correlated Nash Equilibrium:}
    ...
    \blist
        ...
        \item Thus expected return is $0.5 \cdot 6 + 0.5 \cdot 2 = 4$, which is greater than deviating from the action where the agent $i$  has an expected return $0.5 \cdot 0 + 0.5 \cdot 7 = 3.5$
    \elist
    }
\end{flushleft}
...
\fat{Coarse} correlated equilibria are more general than correlated equilibria where the action modifier \(\xi_i\) is not conditioned on the recommended action given by \(\pi_c\).
...
\begin{frame}{Pareto Optimality}
 ...
 To address some of these limitations, we can add additional solution requirements such as \fat{Pareto optimality}. A joint policy \(\pi\) is \fat{Pareto-optimal} if it is not \fat{Pareto-dominated} by any other joint policy.

A joint policy \(\pi\) is \fat{Pareto-dominated} by another policy \(\pi'\) if
 \begin{equation*}
...
\begin{frame}{Complexity of computing equilibria}
  ...
  \begin{itemize}
    ...
    \item Two-agent zero-sum games have \fat{polynomial-time} minimax solutions via linear programming
    \item Correlated equilibria in non-repeated normal-form general-sum games can also be computed in \fat{polynomial time}
    ...
    \item Finding Nash equilibria (NASH problem) is a \fat{total search} problem and has been proven to be\fat{ PPAD complete}, one other PPAD complete problem is the END-OF-LINE Problem
  \end{itemize}
\end{frame}
...
\blist
    \item \fat{Reduction to NASH:} Computing Brouwer fixed points and other PPAD problems are reducible to the NASH problem, indicating there are no known efficient (polynomial time) algorithms for solving NASH
    
    \item \fat{Approximate \(\epsilon\)-Nash Equilibrium:} PPAD-completeness holds for both approximate solutions (\(\epsilon > 0\)) and exact solutions (\(\epsilon = 0\)), with approximations often necessary due to potentially irrational equilibria
    
    \item \fat{Implications for MARL:} MARL algorithms are unlikely to be a silver bullet for finding Nash equilibria efficiently
    
    \item \fat{Research Focus in MARL:} Research often targets identifying exploitable structures in certain game types, but MARL may still require \fat{exponential} time when such structures are unavailable.
\elist
